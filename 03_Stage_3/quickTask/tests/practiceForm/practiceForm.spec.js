...
test.describe('Registro exitoso de un estudiante', () => {
  ...
  test.afterEach(async ({ page }) => {
   ...
   test('Completar exitosamente el formulario de estudiantes', async ({ page }) => {
      ...
      const registerForm = page.locator('#userForm');
    ...
    await test.step('Validar que el título de la página sea ToolsQA', async () => {
        await expect(page).toHaveTitle('DEMOQA')
        });

    await test.step('Validar que el formulario de la página sea visible', async () => {
    ...
    await test.step('Validar contenido del formulario antes de enviar', async () => {
        await expect(firstNameField).toHaveValue('Tomas');
        await expect.soft(hobbieReading).toBeChecked();
        await expect.soft(hobbieMusic).toBeChecked();

        //Validamos que el campo Last Name sea editable
        const readonlyAttr = await lastNameField.getAttribute('readonly');
        await expect(readonlyAttr).toBeNull();

        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        await expect(email).toMatch(emailRegex);
        });
    ...
    await test.step('Validar tabla de confirmación', async () => {
        ...
        await expect(tableRows.nth(0).locator('td').nth(1)).toHaveText(`${firstName} ${lastName}`);
        });
    
  });

  test.skip('Enviar formulario vacío', async ({ page }) => {
